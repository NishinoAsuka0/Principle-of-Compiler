...
bool equal_Type(Type T1, Type T2){
	...
	if(T1->kind == BASIC){
		if(T2->kind == BASIC && (T1->inform.basic == T2->inform.basic)){
			if(DEBUG_FLAG){
				printf("T1 and T2 are both BASIC, T1 is %d and T2 is %d\n",T1->inform.basic, T2->inform.basic);
			}
			return true; // 基本类型相同
		}
		if(DEBUG_FLAG){
...
void ExtDef(struct Node* node)//局部语句块遍历
{
	...
	if(Use_This_Rule(node, 3, "Specifier", "ExtDecList", "SEMI")){
		...
		FunDec(node->firstChild->bro, type, 1);
		CompSt(node->firstChild->bro->bro, type);
	}
	else if(Use_This_Rule(node, 3, "Specifier", "FunDec", "SEMI")){
		if(DEBUG_FLAG)	printf("ExtDef := Specifier FunDec SEMI\n");
		Type type = Specifier(node->firstChild);
		type->possition = RIGHT;        //函数返回值均为右值
                FunDec(node->firstChild->bro, type, 0);
	}
	else{
...
Type Specifier(struct Node*node){
	if(DEBUG_FLAG){
        ...
        if(node == NULL)
                return	NULL;
	if(DEBUG_FLAG){
                printf("Go in Specifier analyse and node is not NULL\n");
        }
...
Type analyseType(struct Node*node){
	...
	if(equal_string(node->Valstr, "float")){
		...
		if(DEBUG_FLAG)  printf("This type is INT!\n");
                newType->kind = BASIC;
...
void ExtDecList(struct Node*node, Type ntype){
	...
	if(Use_This_Rule(node, 1, "VarDec")){
		...
		if(ntype->kind == STRUCTURE)
			VarDec(node->firstChild, ntype, 1, 1);
		else
			VarDec(node->firstChild, ntype, 0, 1);
	}
...
void CompSt(struct Node *node, Type ntype){
	...
	if(Use_This_Rule(node, 4, "LC", "DefList", "StmtList", "RC")){
		...
		StmtList(node->firstChild->bro->bro, ntype);
                ...
                if(DEBUG_FLAG)  printf("CompSt := LC DefList StmtList(=NULL) RC\n");
                DefList(node->firstChild->bro, 0);
        }
	else if(Use_This_Rule(node, 3, "LC", "StmtList", "RC")){
                if(DEBUG_FLAG)  printf("CompSt := LC DefList(=NULL) StmtList RC\n");
                StmtList(node->firstChild->bro, ntype);
        }
	else if(Use_This_Rule(node, 2, "LC", "RC")){
                if(DEBUG_FLAG)  printf("CompSt := LC DefList(=NULL) StmtList(=NULL) RC\n");
        }
	else{
		printf("Error!No This analyse!\n");
	}
...
FieldList DefList(struct Node*node, int ifstruct){
	if(DEBUG_FLAG){
        ...
        if(node == NULL)
                return NULL;
	FieldList newField = Def(node->firstChild, ifstruct);
	if(newField == NULL)
		return DefList(node->firstChild->bro, ifstruct);
        FieldList f;
        for(f = newField; f->next!=NULL; f = f->next);
                f->next = DefList(node->firstChild->bro, ifstruct);
...
FieldList Def(struct Node*node, int ifstruct){
	...
	if(Use_This_Rule(node, 3, "Specifier", "DecList", "SEMI")){
		...
		Type type = Specifier(node->firstChild);
		return DecList(node->firstChild->bro, type, ifstruct);
	}
	else{
                printf("Error!No This analyse!\n");
		return NULL;
...
FieldList DecList(struct Node*node, Type ntype,int ifstruct){
	...
	if(Use_This_Rule(node, 1, "Dec")){
		...
		newField = Dec(node->firstChild, ntype, ifstruct);
		if(newField == NULL)
			return DecList(node->firstChild->bro->bro, ntype, ifstruct);
		FieldList f;
		for(f = newField; f->next!=NULL; f = f->next);
...
FieldList Dec(struct Node*node, Type ntype, int ifstruct){
	if(DEBUG_FLAG){
        ...
        if(Use_This_Rule(node, 1, "VarDec")){
                ...
                return VarDec(node->firstChild, ntype, ifstruct, 1);
        }
        else if(Use_This_Rule(node, 3, "VarDec", "ASSIGNOP", "Exp")){
                if(DEBUG_FLAG)  printf("Dec := VarDec ASSIGNOP Exp\n");
                FieldList newField = VarDec(node->firstChild, ntype, ifstruct, 1);
...
void Stmt(struct Node *node, Type ntype){
	...
	while(child!=NULL){
		if(equal_string("Exp", child->nodeName)){
			...
			Exp(child->bro->bro);
			Stmt(child->bro->bro->bro->bro, ntype);
		}	
		child = child->bro;
	}
}
Type StructSpecifier(struct Node* node){
	...
	if(Use_This_Rule(node, 5, "STRUCT", "OptTag", "LC", "DefList", "RC")){
		...
		if(DEBUG_FLAG){
			printf("Struct node name is %s\n",nodename);
		}
		structType->kind = STRUCTURE;
		Structure newStruct = (Structure)malloc(sizeof(struct Structure_));
		newStruct->name = nodename;
		bool canInsert = true;
		//错误类型16
		if(find(nodename)){
		...
		newStruct->next = DefList(node->firstChild->bro->bro->bro, 1);
                structType->inform.structure = newStruct;
		if(canInsert){
		...
		//structType->inform.structure = newStruct;
                bool canInsert = true;
                //DefList(node->firstChild->bro->bro->bro, 1);
		newStruct->next = DefList(node->firstChild->bro->bro, 1);
                structType->inform.structure = newStruct;
                if(canInsert){
                        if(DEBUG_FLAG)  printf("insert struct and the name is %s\n", nodename);
                        //insert(nodename, structType);
...
Function FunDec(struct Node*node, Type ntype, int state){
	...
	newfunc->type = ntype;
	newfunc->state = state;
	Type nowfuncType = Type_get(node->firstChild);
	//错误类型4
	if(findFunc(newfunc) != 0){
		//printError("4", child->lineNum, child->Valstr);
		if(state == 1 && nowfuncType->inform.function->state == 1){
			printError("4", child->lineNum, "函数出现重复定义");
		}
		else if(!equal_Type(ntype, nowfuncType->inform.function->type)){
			printError("19", child->lineNum, "函数名一致，但返回类型不一致");
		}
	}
	if(Use_This_Rule(node, 4, "ID", "LP", "VarList", "RP")){
		if(DEBUG_FLAG)  printf("FunDec := ID LP VarList RP \n");
		FieldList nextField = VarList(node->firstChild->bro->bro, ntype, state);
	...
	funcType->inform.function = newfunc;
	funcType->possition = RIGHT;
	if(nowfuncType != NULL && !equal_Param(newfunc->next, nowfuncType->inform.function->next)){
		printError("19", child->lineNum, "函数名一致，但形参数量或者形参类型不一致");
	}
	if(nowfuncType != NULL){
		if(state == 1)
			Update(newfunc->name);
...
FieldList VarList(struct Node* node, Type ntype, int state){
	...
	if(Use_This_Rule(node, 3, "ParamDec", "COMMA", "VarList")){
		...
		FieldList newField = ParamDec(node->firstChild, state);
		if(newField == NULL)
			return VarList(node->firstChild->bro->bro, ntype, state);
		FieldList f;
		for(f = newField; f->next != NULL; f = f->next);
		f->next = VarList(node->firstChild->bro->bro, ntype, state);
		return newField;
	}
...
FieldList ParamDec(struct Node*node, int state){
	...
	if(Use_This_Rule(node, 2, "Specifier", "VarDec")){
		...
		newField = VarDec(node->firstChild->bro, newType, 0, state);
		return newField;
	}
...
FieldList VarDec(struct Node*node, Type ntype, int ifstruct, int state){
	...
	if(Use_This_Rule(node, 1, "ID")){
		...
		FieldList newField = (FieldList)malloc(sizeof(struct FieldList_));
                ...
                newField->next = NULL;
		Type ifexist = Type_get(node->firstChild);
		//bool exists = false;
		//if(ifexist != NULL){
			//if(ifexist->kind == BASIC)	printf("BASIC\n");
			//if(ifexist->kind == ARRAY)      printf("ARRAY\n");
			//if(ifexist->kind == FUNCTION)      printf("FUNCTION\n");
			//if(ifexist->kind == STRUCTURE)      printf("STRUCTURE\n");
		//	if(ifexist->kind != FUNCTION){
		//		exists = true;
		//	}
		//}
		//错误类型3
		if((ifstruct == 0) && ifexist != NULL && state == 1){
			printError("3", node->firstChild->lineNum, "变量出现重复定义，或变量与前面定义过的结构体名字重复");
			return newField;
		}
		//错误类型15
		if(ifstruct && ifexist != NULL && state == 1){
		...
		return VarDec(node->firstChild, newArrayType, 0, state);
	}
	else{
                printf("Error!No this analyse!\n");
                return NULL;
...
Type Exp(struct Node*node){
	...
	if(equal_string(Children->nodeName, "ID")){
		...
		//错误类型2
			...
			return typeFunc->inform.function->type;
		}
		else if(Use_This_Rule(node, 3, "ID", "LP", "RP")){
		...
		if(!equal_Type(LeftExp, RightExp)){
			printError("5", Children->bro->lineNum, "(Exp)left and right type is not equal!");
			return NULL;
		}
		//错误类型6
		if(LeftExp->possition == RIGHT){// 右值
		...
		Type LeftExp = Exp(Children);
                Type RightExp = Exp(Children->bro->bro);
		//错误类型7
                if(LeftExp != NULL && RightExp != NULL && !equal_Type(LeftExp, RightExp)){
		...
		//错误类型13
		if(LeftType != NULL && LeftType->kind != STRUCTURE){
		...
		return idType;
                ...
                if(DEBUG_FLAG)  printf("Exp := NOT Exp\n");
                Type ExpType = Exp(Children->bro);
                return ExpType;
        }
	else if(Use_This_Rule(node, 3, "LP", "Exp", "RP")){
                if(DEBUG_FLAG)  printf("Exp := LP Exp RP\n");
                return Exp(Children->bro);
        }
	else if(Use_This_Rule(node, 1, "INT")){
		if(DEBUG_FLAG)  printf("Exp := INT\n");
		Type intType = (Type)malloc(sizeof(struct Type_));
		intType->kind = BASIC;
		intType->inform.basic = 1;
		intType->possition = RIGHT;
		return intType;
	}
	else if(Use_This_Rule(node, 1, "FLOAT")){
                if(DEBUG_FLAG)  printf("Exp := FLOAT\n");
                Type floatType = (Type)malloc(sizeof(struct Type_));
                floatType->kind = BASIC;
                floatType->inform.basic = 0;
                floatType->possition = RIGHT;
                return floatType;
        }
...
void Update(char * name){
	int HashNum = time33_hash(name);
        HashNode curNode = gTable[HashNum];
	HashNode nowNode = curNode;
        while(nowNode != NULL){
                if(strcmp(nowNode->name, name) == 0){
                        nowNode->type->inform.function->state = 1;
                }
                nowNode = nowNode->next;
        }
        gTable[HashNum] = curNode;
}
Type Struct_Type_get(Type structType, char* name){
	FieldList nowField = structType->inform.structure->next;
	if(DEBUG_FLAG){
		printf("struct name is %s\n",structType->inform.structure->name);
	}
	while(nowField != NULL){
		if(DEBUG_FLAG){
			printf("now Field name is %s and desName is %s\n", nowField->name, name);
		}
